...
- [2026-08-01][A] Fix SDT/gia tri (`.row .v`) bi cat nua dong khi khong du cho canh label: nguyen nhan `word-break:break-word` + flexbox khong wrap khien flex ep hep roi be gay chu so. Sua `.row{flex-wrap:wrap}` + `.v{min-width:45%; word-break:normal}` -> khi khong vua, ca cum gia tri nhay nguyen ven xuong dong duoi.
